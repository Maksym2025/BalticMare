...
-- Approved backend foundation: 2026-08-28
-- RLS is enabled here; access policies belong to the next security migration.
...
create table if not exists public.companies (
  ...
  customer_segment_id uuid references public.customer_segments(id),
...
create table if not exists public.product_variants (
  ...
  order_qty_step numeric(12,3) not null default 1,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table if not exists public.samples (
  ...
  currency char(3) not null default 'EUR',
  status text not null default 'requested' check (status in ('requested','approved','paid','shipped','delivered','cancelled')),
  created_at timestamptz not null default now()
);
...
create table if not exists public.sample_credits (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.companies(id),
  sample_id uuid not null unique references public.samples(id),
  amount numeric(12,2) not null check (amount >= 0),
  currency char(3) not null default 'EUR',
  status text not null default 'available' check (status in ('available','applied','expired','cancelled')),
  applied_order_id uuid references public.orders(id),
...
create table if not exists public.payments (
  ...
  method text not null check (method in ('card','paypal','bank_transfer','invoice')),
...
create table if not exists public.documents (
  ...
  order_id uuid references public.orders(id),
  quote_id uuid references public.quotes(id),
  document_type text not null check (document_type in ('invoice','quote','order','delivery_note','company','other')),
...
-- Security boundary: every public table starts protected.
